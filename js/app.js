...
var a = document.querySelector('#sticky'), b = null;  // селектор блока, который нужно закрепить
...
function Ascroll() {
  if (b == null) {  // добавить потомка-обёртку, чтобы убрать зависимость с соседями
    ...
    for (var i = 0; i < Sa.length; i++) {  // перечислить стили CSS, которые нужно скопировать с родителя
      if (Sa[i].indexOf('overflow') == 0 || Sa[i].indexOf('border') == 0 || Sa[i].indexOf('box-shadow') == 0 || Sa[i].indexOf('background') == 0  || Sa[i].indexOf('z-index') == 0) {
	...
	function animOnScroll() {
		for (let index = 0; index < animItems.length; index++) {
         ...
         const animItem = animItems[index];
			...
			const animStart = 2;
         const num = animItem.textContent * 1;
         const id = animItem.id;
			let animItemPoint = window.innerHeight - animItemHeight / animStart;
			if (animItemHeight > window.innerHeight) {
				animItemPoint = window.innerHeight - window.innerHeight / animStart;
			}

			if (pageYOffset > (animItemOffset - animItemPoint) && pageYOffset < (animItemOffset + animItemHeight) && !animItem.classList.contains('_done')) {
				   animItem.classList.add('_done')
               const options = {
                  duration: 0.6, // animation duration in seconds (2)
                  useEasing: false // ease animation (true)
                }
               let countUp = new CountUp(id, num, options);
               countUp.start(); 
